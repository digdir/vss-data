...
def _write_json_file(log_data: Dict[str, Any], file_path: str) -> None:
    ...
    if os.path.exists(file_path_str):
        backup_path = file_path_str.replace(".json", "_backup_log.json")
...
class InstanceTracker:
    ...
    def has_processed_instance(
        self, org_number: str, digitaliseringstiltak_report_id: str
    ) -> bool:
        org_data = self.log_file.get("organisations", {}).get(org_number, {})
        events = org_data.get("events", [])
        for event in events:
            if (
                event.get("event_type") == "initiell_skjema_instance_created"
                and event.get("digitaliseringstiltak_report_id")
    ...
    def logging_instance(
        ...
        if org_number != instance_meta_data["instanceOwner"].get("organisationNumber"):
            raise
        ValueError(
            f"Organization numbers do not match: {org_number} != {instance_meta_data['instanceOwner'].get('organisationNumber')}"
        )

        datamodel_metadata = get_meta_data_info(instance_meta_data["data"])
        instance_log_entry: Dict[str, Any] = {
            ...
            "virksomhets_name": instance_meta_data.get("instanceOwner", {})
            .get("party", {})
            .get("name"),
...
def get_meta_data_info(
    ...
    for instance in list_of_data_instance_meta_info:
        if instance.get("dataType") == "DataModel" and instance.get("contentType") in [
            "application/xml",
            "application/json",
        ]:
            return instance
...
def get_required_key(record: Dict[str, str], key: str) -> str:
    if key not in record:
        raise KeyError(f"Missing required key: {key}")
    return record[key]


def transform_flat_to_nested_with_prefill(
...
def validate_prefill_data(prefill_data_row: Dict[str, Any]) -> bool:
    ...
    if not _is_valid_uuid(str(report_id)):
        raise PrefillValidationError(
            f"Invalid UUID format for digitaliseringstiltak_report_id: {report_id}"
        )
