...
def get_meta_data_info(
    list_of_data_instance_meta_info: List[Dict[str, str]]
) -> Dict[str, str]:
    ...
    for instance in list_of_data_instance_meta_info:
        if instance.get("dataType") in ["DataModel", "model"] and instance.get(
            "contentType"
        ) in ["application/xml", "application/json"]:
            return instance
        else:
            continue

    raise ValueError(
        "No instance with dataType='DataModel' and contentType='application/xml' or 'application/json' was found."
    )


def extract_instances_ids(data_storage_extract: Dict[str, Any]) -> List[Dict[str, Any]]:
    instances = []
            ...
            instances.append(
                {
                    ...
                    "organisationNumber": instance["instanceOwner"].get(
                        "organisationNumber", ""
                    ),
...
def make_api_call(
    ...
    files: Optional[Dict[str, Tuple[str, str, str]]] = None,
) -> requests.Response:
    try:
        ...
        logging.error(
            f"HTTP error {response.status_code} on {method} {url}: {response.text}"
        )
        raise
...
def mock_update_substatus(
    instanceOwnerPartyId: str, instanceGuid: str, digitaliseringstiltak_report_id: str
) -> Mock:
    now_iso = dt.datetime.utcnow().isoformat() + "Z"
    # Construct mock response data
    response_data = {
        "instanceOwner": {"partyId": instanceOwnerPartyId},
...
def mock_post_new_instance(
    header: Optional[Dict[str, str]], files: Dict[str, Tuple[str, str, str]]
) -> Dict[str, Any]:
    ...
    response_data = {
        ...
        "instanceOwner": {
            ...
            "party": {
                ...
                "unitType": "AS",
                "name": datamodel_content.get("Prefill", {})
...
class AltinnInstanceClient:
    ...
    def get_instance(
        ...
        return make_api_call(
            method="GET", url=url, headers=self._get_headers("application/json")
        )
    ...
    def get_active_instance(
        self, instanceOwnerPartyId: str, header: Optional[Dict[str, str]] = None
    ) -> Optional[requests.Response]:
    ...
    def mock_test_post_new_instance(
        self,
        files: Dict[str, Tuple[str, str, str]],
        header: Optional[Dict[str, str]] = None,
    ) -> Dict[str, Any]:
    ...
    def get_stored_instances_ids(
        ...
        data_storage_instances = make_api_call(
            method="GET",
            url=url,
            headers=self._get_headers("application/json"),
            params=params,
        )
        if data_storage_instances is None:
            raise RuntimeError("API call failed; response is None")

        return extract_instances_ids(data_storage_instances.json())

    def instance_created(
        self, org_number: str, tag: str, header: Optional[Dict[str, str]] = None
    ) -> bool:
        stored_instances = self.get_stored_instances_ids(
            self._get_headers("application/json")
        )
    ...
    def mock_test_update_substatus(
        self,
        instanceOwnerPartyId: str,
        instanceGuid: str,
        ...
        return mock_update_substatus(
            instanceOwnerPartyId, instanceGuid, digitaliseringstiltak_report_id
        )
